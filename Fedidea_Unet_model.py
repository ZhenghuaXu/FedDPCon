...
def train(args, snapshot_path):
    semi_dice_writer = SummaryWriter(log_dir=f'logs/dice')
    semi_loss_writer = SummaryWriter(log_dir=f'logs/loss')
...
#------------------------------------------------------------------------------Step2-----------------------------------------------------------------------------------------
    ...
    for cur_comm in range(args.num_comm):
        ...
        semi_loss_writer.add_scalar('global' , epoch_loss, global_step=cur_comm)
        with torch.no_grad():
            if (cur_comm + 1) % (1) == 0:
                mean_dice=0.0
                ...
                for number,(val_client, db) in enumerate(zip(val_list.values(),db_val.values())):
                    ...
                    for i,inner_list in enumerate(metric_list):
                        ...
                        if i==0:
                            print('number %d : iteration %d : dice : %f hd95 : %f assd : %f ppv : %f ' % (number,cur_comm, dice_performance, hd95, assd, ppv))
                        if number==3:
                            print('mean dice = ', mean_dice/4)
                            if mean_dice/4 > best_performance:
                                    best_performance = mean_dice/4
                                    if (len(lab_ListClinet)==1):  
                                        save_mode_path = os.path.join("/mnt/sdd/PTH/HAM",
                                                            'Fedidea_Local_Only1__{}.pth'.format( round(best_performance, 4)))
                                    elif (len(lab_ListClinet)==2):  
                                        save_mode_path = os.path.join("/mnt/sdd/PTH/HAM",
                                                            'Fedidea_Local_Only2__{}.pth'.format( round(best_performance, 4)))
